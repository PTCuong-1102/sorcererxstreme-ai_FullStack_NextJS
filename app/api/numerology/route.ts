...
export async function POST(req: Request) {
  try {
    ...
    // Extract Wikipedia search terms using advanced logic for numerology
    const searchTerms = extractSearchTermsAdvanced(
      fullUserContext, 
      'numerology',
      { numbers, type: type || mode }
    );
    
    // Get Wikipedia content for fact-checking
    console.log('Fetching Wikipedia content for numerology:', searchTerms);
    ...
    if (validWikipediaContents.length > 0) {
      console.log('Performing fact-check with Wikipedia sources for numerology');
